...
object SyncManagerUtils {
    ...
    fun findIndexOfChannel(
        ...
        for (i in channels.indices) {
            val c: GroupChannel = channels[i]
            if (c.url.equals(targetChannel.url)) {
                return i
            }
    ...
    fun getIndexOfChannel(channels: List<GroupChannel>, targetChannel: GroupChannel): Int {
        for (i in channels.indices) {
            if (channels[i].url.equals(targetChannel.url)) {
                return i
            }
        }
        return -1
    ...
    fun findIndexOfMessage(messages: List<BaseMessage>, newMessage: BaseMessage): Int {
        ...
        for (i in 0 until messages.size - 1) {
            val m1: BaseMessage = messages[i]
            val m2: BaseMessage = messages[i + 1]
    ...
    fun getIndexOfMessage(messages: List<BaseMessage>, targetMessage: BaseMessage): Int {
        for (i in messages.indices) {
            val msgId1: Long = messages[i].messageId
            val msgId2: Long = targetMessage.messageId
            if (msgId1 == msgId2) {
                if (msgId1 == 0L) {
    ...
    private fun getRequestId(message: BaseMessage): String {
        ...
        if (message is UserMessage) {
            return (message as UserMessage).requestId
        }
    ...
    val myUserId: String
        get() = if (SendBird.getCurrentUser() == null) {
